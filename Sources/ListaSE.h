...
template<class T> T ListaSE<T>::obtenerRandom(){

    this->dado = new Dado(this->tamanio);
    if (tamanio<2){
        return obtenerElemento(0);
    }
    int tirada = dado->lanzar()-1;
    delete this->dado;
    return obtenerElemento(tirada);
...
template<class T> T ListaSE<T>::obtenerElemento(unsigned int posicion) {
